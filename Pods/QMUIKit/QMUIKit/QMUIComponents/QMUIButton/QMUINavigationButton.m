    ...
    dispatch_once(&onceToken, ^{
        ...
        if (@available(iOS 11, *)) {
            ExtendImplementationOfVoidMethodWithoutArguments([UINavigationBar class], @selector(layoutSubviews), ^(UINavigationBar *selfObject) {
                ...
                if (contentView) {
                    if (@available(iOS 13, *)) {
                        UIEdgeInsets margins = contentView.layoutMargins;
                        contentView.frame = CGRectMake(-margins.left, -margins.top, margins.left + margins.right + contentView.frame.size.width, margins.top + margins.bottom + contentView.frame.size.height);
                    } else {
